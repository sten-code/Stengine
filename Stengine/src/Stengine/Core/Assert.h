...
#include <filesystem>
	...
	#define ST_INTERNAL_ASSERT_GET_MACRO(...) ST_EXPAND_MACRO( ST_INTERNAL_ASSERT_GET_MACRO_NAME(__VA_ARGS__, ST_INTERNAL_ASSERT_WITH_MSG, ST_INTERNAL_ASSERT_NO_MSG) )

	// Currently accepts at least the condition and one additional parameter (the message) being optional
	#define ST_ASSERT(...) ST_EXPAND_MACRO( ST_INTERNAL_ASSERT_GET_MACRO(__VA_ARGS__)(_, __VA_ARGS__) )
	#define ST_CORE_ASSERT(...) ST_EXPAND_MACRO( ST_INTERNAL_ASSERT_GET_MACRO(__VA_ARGS__)(_CORE_, __VA_ARGS__) )
#else
	#define ST_ASSERT(...)
	#define ST_CORE_ASSERT(...)
#endif